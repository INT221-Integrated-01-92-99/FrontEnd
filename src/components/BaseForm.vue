  ...
  <div class="form bg-cream-light pt-8 pb-14 bg-fixed">
    <form class="w-5/6 2xl:w-3/5 mx-auto">
      <base-card class="w-full mx-auto">
        ...
        <div
          class="flex flex-col 2xl:space-x-14 lg:space-x-16"
        >
          ...
          <div class="lg:space-y-3 space-y-4 flex flex-col 2xl:pr-16 lg:pr-24 md:px-10">
            <div class="flex flex-col">
              ...
              >
                <option v-for="brand in brandArray" :key="brand.idBrand" :value="brand">
                  {{ brand.brandName }}
                </option>
              </select>
            </div>
        ...
        <div
          class="flex justify-center lg:pb-11 pb-7 lg:pt-10 pt-7"
        >
          <div class="space-x-5">
            <base-button
...
export default {
  ...
  methods: {
    checkForm() {
      ...
      this.PriceInput = this.enterPrice === "" || this.enterPrice % 1 != 0 || this.enterPrice < 0;
    ...
    submitForm() {
      ...
      try {
        ...
        this.productArray = this.productArray.map((m) =>
          m.id === data.id
            ? { ...m,
            probrandName: editPro.brandName,
    ...
    selectColor(color) {
      if (this.product.item.map((c) => c.color.idColor).includes(color.idColor)) {
        this.product.item = this.product.item.filter(
          (c) => c.color.idColor != color.idColor
        );
      } else {
        this.product.item.push(color);
      }
    },
    async fetch(url) {
      try {
  ...
  async created() {
    ...
    this.productArray = await this.fetch("http://localhost:5000/product/");
    if (!isNaN(this.proId)) {
      ...
      this.product.item = product.item;
    }
  },
  watch: {
